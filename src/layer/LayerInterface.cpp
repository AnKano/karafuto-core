...
namespace KCore {
    LayerInterface::LayerInterface
            () : LayerInterface(0.0f, 0.0f) {}

    LayerInterface::LayerInterface
            (float latitude, float longitude) : mLayer(latitude, longitude) {}

    LayerInterface::LayerInterface
            (float latitude, float longitude, const char *url) : mLayer(latitude, longitude) {
        setLayerRasterUrl(url);
    }

    void LayerInterface::updateProjectionMatrix
            (const glm::mat4 &projectionMatrix) {
        mCameraProjectionMatrix = projectionMatrix;
    }

    void LayerInterface::updateProjectionMatrix
            (const float *projectionMatrix_ptr, bool transpose) {
    ...
    void LayerInterface::updateViewMatrix
    ...
    void LayerInterface::updateViewMatrix
            (const float *viewMatrix_ptr, bool transpose) {
    ...
    void LayerInterface::updateViewMatrixFromParams
            (const glm::vec3 &positionVector, const glm::vec3 &rotationVector) {
    ...
    void LayerInterface::updateViewMatrixFromParams
            (const float *positionVector_ptr, const float *rotationVector_ptr) {
    ...
    void LayerInterface::update
            (const float *projectionMatrix_ptr, const float *viewMatrix_ptr,
    ...
    void LayerInterface::setLayerRasterUrl
            (const char *url) {
    ...
    void LayerInterface::updateProjectionMatrixFromParams
    ...
    DllExport KCore::LayerInterface *CreateTileLayer
            (float latitude, float longitude) {
        return new KCore::LayerInterface(latitude, longitude);
    }

    DllExport KCore::LayerInterface *CreateTileLayerWithURL
            (float latitude, float longitude, const char *url) {
        return new KCore::LayerInterface(latitude, longitude, url);
    }

    DllExport void Update
            (KCore::LayerInterface *layer_ptr,
    ...
    DllExport void UpdateViewMatrixFromParams
            (KCore::LayerInterface *layer_ptr,
    ...
    DllExport std::vector<LayerEvent> *GetCoreEventsVector
            (KCore::LayerInterface *layer_ptr) {
        return new std::vector<LayerEvent>(layer_ptr->getCoreEvents());
    }

    DllExport std::vector<LayerEvent> *GetImageEventsVector
            (KCore::LayerInterface *layer_ptr) {
        return new std::vector<LayerEvent>(layer_ptr->getImageEvents());
    }

    DllExport LayerEvent *EjectEventsFromVector
            (std::vector<LayerEvent> *vector_ptr, int &length) {
        length = (int) vector_ptr->size();
        return vector_ptr->data();
    }

    DllExport void ReleaseEventsVector
