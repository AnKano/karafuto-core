...
namespace kcore {
    data_tile::data_tile(const std::string &quadcode) : quadcode(quadcode) {
        if (quadcode.empty()) throw std::invalid_argument("quadcode cannot be empty!");
    ...
    void data_tile::set_parent(const data_tile *parent) {
        ...
        if (parent == nullptr)
            set_type(data_tile_type::root);
        else
            set_type(data_tile_type::leaf);
    }
    ...
    float data_tile::get_side() {
        ...
        return glm::length(first_point - second_point);
    }
    ...
    enum data_tile_cardinals data_tile::eject_cardinals() {
        switch (quadcode.back()) {
            ...
            default: throw std::runtime_error("Unexpected tile cardinals");
        }
    }
}