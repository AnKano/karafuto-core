...
namespace KCore::opengl {
    ...
    public:
        Mesh(MeshDescription &mesh) {
            ...
            //kren::engine::log(Tag, "Initial Mesh buffers successfully declared", info);

            combineBuffers();

            //kren::engine::log(Tag, "Initial Mesh buffers successfully instantiated", info);
        }
    ...
    private:
        void combineBuffers() {
            if (VAOId) {
                //kren::engine::log(Tag, "Attempt to declare already declared Mesh!", info);
                return;
            }
        ...
        static std::unordered_map<uint16_t, AttributeDescription *> createVertexBuffers(MeshDescription &mesh) {
            ...
            for (auto &attribDesc: describedAttributes) {
                ...
                //kren::engine::log(Tag, "Vertices buffer declared " + additional, info);
        ...
        uint32_t createIndexBuffer(MeshDescription &mesh) {
            ...
            std::string additional = "| buffer id: " + std::to_string(bufferId);
            //kren::engine::log(Tag, "Indices buffer declared " + additional, info);
