...
namespace KCore {
    ...
    private:
        ...
        std::shared_ptr<KCore::opengl::Shader> Shader;
        std::shared_ptr<KCore::opengl::Mesh> Mesh;
    ...
    public:
        TileRenderer() {
            ...
            OffscreenContext = glfwCreateWindow(512, 512, "", nullptr, nullptr);
            ...
            Shader = std::make_shared<KCore::opengl::Shader>("TileRenderer");
            Shader->addShaderPairFromPath("assets/shaders/plane");
            Shader->build();
        ...
        void load_to_texture() {
            ...
            while (!glfwWindowShouldClose(OffscreenContext)) {
                glClear(GL_COLOR_BUFFER_BIT);

                Shader->bind();
                int32_t uScaleMatrixPos = Shader->uniform_position("u_scale_matrix");
                int32_t uTranslationMatrixPos = Shader->uniform_position("u_translation_matrix");
                int32_t uColorPos = Shader->uniform_position("u_color");
                ...
                glfwSwapBuffers(OffscreenContext);
                glfwPollEvents();
            }
        }

        void read_from_texture() {
            glBindTexture(GL_TEXTURE_2D, Texture);
